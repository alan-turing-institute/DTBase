"""
A module for the model dashboard actions
"""
...
def fetch_model_runs(model_name, dt_from, dt_to):
    """Get all runs for a given model from the database.

    Args:
        model_name: The name of the sensor type.
    Returns:
        List of dictionaries, one for each run.
    """
    payload = {"model_name": model_name, "dt_from": dt_from, "dt_to": dt_to}
    response = utils.backend_call("get", "/model/list_model_runs", payload=payload)
...
def fetch_prediction_data(run_id):
    """Get the data from a prediction run

    Args:
        run_id

    Returns:
        pandas DataFrame with columns for each measure and for timestamp.
    """
    result = {}
...
def index():
    ...
    is_valid_sensor_type = sensor_type_name in [s["name"] for s in sensor_types]
    if (
        dt_from is None
        or dt_to is None

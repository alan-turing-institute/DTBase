...
def plot_data(xtrain_timestamp, xtrain, xtest_timestamp, xtest, plot_label, save_path=None, save_suffix=''):
...
def hodmd_pipeline(timestamps, xdata, labels, hodmd_d=250, train_test_split=0.8, save_path=None, save_suffix=''):
    ...
    plot_data(xtrain_timestamp, xtrain_normalized, xtest_timestamp, xtest_normalized, labels_, \
        save_path, save_suffix) 

    snapshots = xtrain_normalized.T
    model = HODMD(svd_rank = 0.99, svd_rank_extra = 0.9, exact = True, \
        opt = True, d = hodmd_d, forward_backward = True, sorted_eigs = 'real')
    model = model.fit(snapshots)
    filename = None if save_path is None else save_path + '/eigs_before_mode_tuning{0}.png'.format(save_suffix)
    ...
    model = tunedDMD
    filename = None if save_path is None else save_path + '/eigs_after_mode_tuning{0}.png'.format(save_suffix)
    plot_eigs(model, show_axes=True, show_unit_circle=True, filename=filename)

    xreconstructed = model.reconstructed_data.real

    if save_path is not None:
        suffixes = [save_suffix] if len(labels) == 1 else [save_suffix +'_'+ label for label in labels]
        for idx, label in enumerate(labels):
            fig, ax = plt.subplots(figsize=(17, 4.5))
            ax.plot(xtrain_timestamp, xtrain_normalized[ : , idx], label=label + '_train', c='k', \
                linewidth = 1)
            ax.plot(xtest_timestamp, xtest_normalized[ : , idx], label=label + '_forecast', c='k', \
                linewidth = 1)
            ax.plot(xtrain_timestamp, xreconstructed[idx, : num_train_samples].T, \
                label='prediction_train', c='r', linewidth = 1)
            ax.plot(xtest_timestamp, xreconstructed[idx, num_train_samples : ].T, ':', \
                label='prediction_forecast', c='r', linewidth = 1)
            ax.legend(loc='best')
